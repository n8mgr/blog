---
{
  ...
  "description": "How SIMD helped us build a wire-compatible Reed-Solomon library that's up to 25x faster in native Rust and 8x faster in the browser.",
  "tags": ["rust", "sia"]
}
---
...
Reconstruct throughput is reported per data slab (`data_shards` x `shard_size`), which makes it comparable to download throughput. It also means `reconstruct -1 data lost` looks very high, since only one shard is rebuilt while the rate is normalized to the full slab.
...
That's roughly 8x faster on encode. The reconstruct gap depends on how much is missing: with one shard gone it's under 2x, but in the worst case, all 10 data shards lost, `reed_solomon_erasure` is over 5x slower.
